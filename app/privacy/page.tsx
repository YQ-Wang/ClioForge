...
import { CanwooBrand } from '@/components/canwoo-brand';
...
export default async function Privacy() {
  ...
  const sections = [
    [
      L('你带入 Canwoo 的信息', 'Information you bring to Canwoo'),
      L(
    ...
    [
      ...
      L(
        '连接 Drive 时，Canwoo 请求访问你通过文件选择器选择的文件。Google 将该权限描述为读取和修改所选文件；Canwoo 当前只读取和导入，不修改云端原件。我们保存文件副本、名称、来源链接及版本信息，用于阅读、引用和研究复现。Google 文档、表格和演示文稿会导出为 PDF。',
        'When you connect Drive, Canwoo requests access to files you select through the file picker. Google describes this permission as reading and modifying selected files; Canwoo currently reads and imports them without editing cloud originals. We save a copy, its name, source link and version information for reading, citation and reproducibility. Google documents, spreadsheets and presentations are exported as PDFs.',
      ),
    ],
    [
      L(
      ...
      L(
        '当你明确启动智能转录或分析时，所选材料和研究问题会发送给你选择的模型服务商。通过 OpenRouter 调用时，内容还会由其路由到模型服务商。请先了解所选服务商的数据政策。Canwoo 不将你的研究资料用于广告、出售或训练自有通用模型。Cloudflare 提供应用运行、数据库、文件保存和账户邮件服务。',
        'When you start assisted transcription or analysis, your selected sources and question are sent to the model provider you choose. OpenRouter requests are also routed to its model providers. Review your chosen provider’s data policy before use. Canwoo does not use your research sources for advertising, sale or training its own general-purpose models. Cloudflare provides application hosting, databases, file storage and account email services.',
      ),
    ],
    [
  ...
  return (
    <div className="public-shell">
      ...
      <main className="privacy-page">
        <p className="eyebrow">
          CANWOO · {L('你的材料，由你掌握', 'YOUR SOURCES, YOUR CHOICES')}
        </p>
