  ...
  const citations: WritingCitation[] = (snapshot.evidence as Evidence[]).map(
    (e) => {
      ...
      return {
        ...
        text: `${entry ? footnote(entry, String(e.page)).trim() : `${source?.title || 'Source'}, p. ${e.page}.`} “${e.quote}” (Canwoo version ${version?.revision || '?'}; ${e.version_id})`,
  ...
  for (const version of snapshot.source_versions as SourceVersion[]) {
    ...
    for (const page of version.pages) {
      ...
      citations.push({
        ...
        text: `${entry ? footnote(entry, String(page.page)).trim() : `${source.title}, p. ${page.page}.`} (Canwoo version ${version.revision}; ${version.id})`,
        stale: (snapshot.source_versions as SourceVersion[]).some(
