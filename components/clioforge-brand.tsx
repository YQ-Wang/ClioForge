import hanWordmark from '@/lib/brand-han.json';
import {
  compactFanArt,
  fanArt,
  wordmarkPaths,
  wordmarkViewBox,
} from '@/lib/brand-art';

export function ClioForgeMark({
  className = '',
  compact = false,
}: {
  className?: string;
  compact?: boolean;
}) {
  const art = compact ? compactFanArt : fanArt;
  return (
    <svg className={className} viewBox="0 0 112 72" aria-hidden="true">
      {art.folds.map((fold, index) => (
        <path
          key={index}
          d={fold.d}
          fill={'shade' in fold ? '#201b12' : 'currentColor'}
          opacity={fold.opacity}
        />
      ))}
      <path
        d={art.ribs}
        fill="none"
        stroke="currentColor"
        strokeWidth={compact ? 1.3 : 0.8}
        strokeLinecap="round"
      />
      <path
        d={art.guard}
        fill="none"
        stroke="currentColor"
        strokeWidth={compact ? 2.2 : 1.65}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <circle cx="56" cy="66" r="1.8" fill="currentColor" />
    </svg>
  );
}

export function ClioForgeWordmark() {
  return (
    <svg className="brand-latin" viewBox={wordmarkViewBox} aria-hidden="true">
      <g fill="currentColor">
        {wordmarkPaths.map((d, i) => (
          <path key={i} d={d} />
        ))}
      </g>
    </svg>
  );
}

// Qiji woodblock lettering, generated by scripts/build-han-wordmark.py.
// SIL OFL 1.1; see public/brand/Qiji-OFL.txt. The lockup provides its name.
export function ClioForgeHanWordmark() {
  return (
    <svg
      className="brand-chinese"
      viewBox={hanWordmark.viewBox}
      aria-hidden="true"
    >
      <g fill="currentColor">
        {hanWordmark.paths.map((d, index) => (
          <path key={index} d={d} />
        ))}
      </g>
    </svg>
  );
}

export function ClioForgeBrand({ tagline }: { tagline?: string }) {
  return (
    <span
      className={`clioforge-brand${tagline ? ' has-tagline' : ''}`}
      aria-label="ClioForge 參伍"
    >
      <ClioForgeMark className="brand-mark" compact />
      <span className="brand-copy">
        <span className="brand-wordmark" aria-hidden="true">
          <ClioForgeWordmark />
          <ClioForgeHanWordmark />
        </span>
        {tagline && <small>{tagline}</small>}
      </span>
    </span>
  );
}
export function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" width="19" height="19" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M21.6 12.23c0-.71-.06-1.39-.18-2.05H12v3.88h5.38a4.6 4.6 0 0 1-1.99 3.02v2.51h3.23c1.89-1.74 2.98-4.31 2.98-7.36Z"
      />
      <path
        fill="#34A853"
        d="M12 22c2.7 0 4.96-.9 6.62-2.41l-3.23-2.51c-.9.6-2.04.96-3.39.96-2.61 0-4.82-1.76-5.61-4.12H3.05v2.59A10 10 0 0 0 12 22Z"
      />
      <path
        fill="#FBBC05"
        d="M6.39 13.92a6.02 6.02 0 0 1 0-3.84V7.49H3.05a10 10 0 0 0 0 9.02l3.34-2.59Z"
      />
      <path
        fill="#EA4335"
        d="M12 5.96c1.47 0 2.79.51 3.82 1.51l2.87-2.87A9.6 9.6 0 0 0 12 2a10 10 0 0 0-8.95 5.49l3.34 2.59C7.18 7.72 9.39 5.96 12 5.96Z"
      />
    </svg>
  );
}
