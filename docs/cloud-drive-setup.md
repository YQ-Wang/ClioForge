...
以下步骤由 Canwoo 运营者配置一次，研究者不需要创建 Google Cloud 项目或提供 OAuth 密钥。Google 登录只请求 `openid`、`email`、`profile`；第一次导入 Drive 资料时才请求 `drive.file`。
...
4. Data Access：保留基本身份权限，并添加 `https://www.googleapis.com/auth/drive.file`。它是 Google 推荐的非敏感文件级权限。对外发布仍需满足 Google 的品牌/基础审核要求；测试授权可能到期，需要重新连接。不要添加整个云盘的权限。Branding 的隐私页面使用 `https://canwoo.com/privacy`。
...
9. 保存并部署配置。在 Canwoo 登录页面选择 **使用 Google 继续**；在研究项目中选择 **导入资料 → Google Drive**。授权后回到原项目选择文件导入。PDF、照片、文本、Google Docs / Sheets / Slides 均可选择；Google 原生文档导出为 PDF。每份文件上限 20 MiB，Google 原生文档导出还受 Google 的导出限制约束。

## English checklist

This is a one-time operator setup. Enable Drive API and Picker API in one Google Cloud project. Configure OAuth Branding/Audience, identity scopes and `drive.file`, add your test account, and create a Web application OAuth client with origin `https://canwoo.com` and both exact callbacks above. Create a website-restricted Picker API key allowing `https://canwoo.com/*` and `https://docs.google.com/*`, restricted to Drive API and Picker API. Configure the four Worker variables listed above using the same project's numeric project number. Researchers use Continue with Google to sign in and Import sources → Google Drive inside their project.

## 2026-09-05 deployment checkpoint

- Project `canwoo`, project number `478761547428`, organization `wanggu.org`.
- OAuth application `Canwoo 参伍` created in External / Testing mode with owner-approved policy acceptance. Web client has the origin and both callbacks above.
- `GOOGLE_DRIVE_CLIENT_ID`, `GOOGLE_DRIVE_CLIENT_SECRET` and `GOOGLE_CLOUD_PROJECT_NUMBER` uploaded to Canwoo Cloudflare Secrets. No secret values belong in this document.
- Drive API enabled. Picker API enable action was submitted; successful activation still needs console verification. Restricted Picker API key, test users, declared scopes and branding URLs still need completion.
- Production sign-in endpoint returns a Google authorization URL with only `email profile openid`. This checks authorization initiation, not an actual Google account login or Drive file import.
- Browser work stopped at a locked Mac. Complete console configuration and real consent/import validation after the owner unlocks it. Publishing/branding verification has not been completed.

## 本地开发与凭据保护 / Local development and credentials

2026-09-05 later checkpoint: Drive and Picker APIs, restricted Picker key, declared identity/`drive.file` scopes, branding/home/privacy URLs and two owner-controlled test accounts are now configured. All four Worker settings are present. Real consent, return to project, switching the Drive account and importing one specifically selected public PDF succeeded. Earlier pending/locked-browser notes above are historical. Google OAuth remains **Testing**; publishing to all Google users is awaiting explicit owner approval after automatic approval review rejected that expansion. This is separate from deploying the Canwoo application itself.
