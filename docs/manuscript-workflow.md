...
- Every substantive paragraph must reference a selected claim and an associated excerpt. New manuscript calls select numbered dossier excerpts; ClioForge supplies their exact quotation, version and page. References are checked against the frozen evidence and source text; unknown references, invented quotation strings and unsupported reference indices are rejected. Repeated citations reuse the same reference number across chapters.
